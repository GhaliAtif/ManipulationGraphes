...
inline void CGraphe::GRAModifierSommet(unsigned int uiNumero, CSommet & SOMParam)
{
	...
	unsigned int * puiListeSourcesArcsArrivants = GEGChercherIndiceSourceArcsArrivants(SOMParam);
	for (unsigned int uiArcArrivant = 0; uiArcArrivant < SOMParam.SOMLireNombreArcsArrivants(); uiArcArrivant++)
	{
		/* Dans ce sommet source, on cherche l'indice de liste de l'arc partant pointant sur SOMParam */
		uiIndiceArcPartantPointantSOMParam = pSOMGRAListeSommets[puiListeSourcesArcsArrivants[uiArcArrivant]]->GEARechercherIndiceArcPartant(SOMParam.SOMLireNumero());

		/* Dans ce sommet source, on change la destination de cet arc partant par "uiNumero" */
		pSOMGRAListeSommets[puiListeSourcesArcsArrivants[uiArcArrivant]]->SOMLireListeArcsPartants(uiIndiceArcPartantPointantSOMParam)->ARCModifierNumeroDestination(uiNumero);
	}
...
inline void CGraphe::GRAAjouterArc(unsigned int uiDepart, CArc & ARCParam)
{
	...
	CArc* ARCArcAAjouter = new CArc(pSOMGRAListeSommets[uiIndiceSommetArrivee]->SOMLireNumero());
	pSOMGRAListeSommets[uiIndiceSommetArrivee]->SOMAjouterArcArrivant(*ARCArcAAjouter);
...
************************************************************************************************************************************************/
inline void CGraphe::GRAModifierArc(unsigned int uiNouveauDepart, unsigned int uiNouvelleArrivee, unsigned int uiAncienDepart, CArc & ARCParam)
{
	/* Un des 3 sommets n'existe pas */
	unsigned int uiIndiceNouveauDepart = GEGRechercherIndiceSommet(uiNouveauDepart);
	unsigned int uiIndiceAncienDepart = GEGRechercherIndiceSommet(uiAncienDepart);
	unsigned int uiIndiceNouvelleArrivee = GEGRechercherIndiceSommet(uiNouvelleArrivee);


	if ((uiIndiceNouveauDepart == EXC_SOMMET_INEXISTANT) || (uiIndiceNouvelleArrivee == EXC_SOMMET_INEXISTANT) || (uiIndiceAncienDepart == EXC_SOMMET_INEXISTANT))
	{
		CException EXCErreur(EXC_SOMMET_INEXISTANT, "Un des 3 sommets n'existe pas : impossible de modifier.");
		throw(EXCErreur);
	}
	...
	try
	{
		...
		GRASupprimerArc(uiAncienDepart, ARCParam);

		/*... et on ajoute l'arc modifie */
		ARCParam.ARCModifierNumeroDestination(uiNouvelleArrivee);
		GRAAjouterArc(uiNouveauDepart, ARCParam);
	}
...
inline void CGraphe::GRASupprimerArc(unsigned int uiDepart, CArc & ARCParam)
{
	...
	if ((pSOMGRAListeSommets[uiIndiceSommetDepart]->GEARechercherIndiceArcPartant(ARCParam.ARCLireNumeroDestination()) == EXC_ARC_INEXISTANT) 
		&& (pSOMGRAListeSommets[uiIndiceSommetArrivee]->GEARechercherIndiceArcArrivant(pSOMGRAListeSommets[uiIndiceSommetArrivee]->SOMLireNumero()) == EXC_ARC_INEXISTANT))
	{
	...
	pSOMGRAListeSommets[uiIndiceSommetArrivee]->SOMSupprimerArcArrivant(*pSOMGRAListeSommets[uiIndiceSommetArrivee]->SOMLireListeArcsArrivants(0));
